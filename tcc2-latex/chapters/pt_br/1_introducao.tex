...
A adoção generalizada das tecnologias de Internet das Coisas (IoT), sistemas de monitoramento industrial e aplicações financeiras em tempo real gerou um volume sem precedentes de dados
temporais, criando desafios críticos para as infraestruturas de armazenamento e análise.
Conforme projetado em 2015 por~\cite{thierer2015iot}, o número de dispositivos IoT conectados vem
...
O gráfico apresenta dados históricos até 2015 e projeções de crescimento até 2019, estimativas que, como confirmado pela Cisco em estudos subsequentes, se mostrariam acertadas.
...
processar fluxos contínuos de dados. Entre eles: o setor financeiro, que utiliza essa capacidade para analisar flutuações de ações e volumes de transações; os serviços de transporte, que a empregam para monitorar viagens e migrações sazonais; a medicina, que a utiliza para rastrear a disseminação de doenças; e a análise meteorológica, que aproveita
...
temporais~\citep{InfluxDataTimeSeries}. Os
bancos de dados relacionais tradicionais sofrem de ineficiências de indexação e limitações de
memória ao lidar com grandes volumes de dados temporais.
Os BDSTs, por sua vez, mantêm alto desempenho por terem sido construídos com o tempo
como preocupação central~\citep{naqvi2017influxdb}. Além da vazão,
os BDSTs oferecem mecanismos especializados de compressão, co-localização de dados em
intervalos de tempo e políticas automáticas de retenção, que reduzem os custos operacionais~\citep{Dunning2014}.
Esses recursos são importantes para aplicações IoT, nas quais medições contínuas de sensores demandam
escalabilidade e uso econômico de recursos.
...
O \textit{Apache IoTDB}~\citep{wang2023iotdb}, desenvolvido na Universidade Tsinghua, foi
concebido especificamente para IoT industrial, utilizando um formato de arquivo colunar
personalizado (TsFile) e um protocolo binário Remote Procedure Call (RPC) que possibilita taxas de ingestão
...
temporais, oferecendo compatibilidade com SQL e o ecossistema
relacional completo como contrapartida a uma menor vazão de escrita. Cada abordagem
...
falta de reprodutibilidade. Uma análise dos estudos comparativos recentes revela que grande parte
...
hardwares ou versões de software. Este trabalho aborda diretamente essa lacuna tornando
ambiente completo de benchmark (i.e., todos os scripts de orquestração, definições de
...
proeminência na literatura: InfluxDB como o padrão de referência
...
Os resultados mostram que nenhum banco de dados domina em todas as dimensões: o IoTDB lidera em
vazão de escrita sequencial e filtragem de valor, o TimescaleDB oferece latência constante em
subamostragem e compatibilidade SQL completa, e o InfluxDB ocupa um meio-termo prático entre
os dois.
...
trabalho e o ambiente reprodutível.
