...
expressão Flux. A presença frequente do InfluxDB em benchmarks anteriores o torna um ponto de referência útil: qualquer novo estudo que o omita perde comparabilidade com a literatura existente.
...
arquivo colunar personalizado (TsFile) e um protocolo binário Thrift RPC (Session API). A ingestão de escritas é armazenada em buffer no heap gerenciado pela
JVM antes de ser descarregada para o disco como segmentos colunares ordenados, possibilitando
vazão excepcionalmente alta para fluxos de séries temporais ordenadas. Sua rápida ascensão em benchmarks recentes, combinada com sua abordagem arquitetural distinta, tornou-o adequado para este estudo.
...
Algumas limitações do iot-benchmark merecem registro. A ferramenta gera dados sintéticos com
distribuição uniforme de valores de ponto flutuante, o que não reproduz fielmente padrões de
dados reais, como sazonalidade, correlação entre sensores ou anomalias pontuais; os resultados
refletem portanto padrões de carga controlados, não implantações de produção. Os parâmetros de
configuração, especialmente o número de dispositivos, a contagem de sensores por dispositivo e a
taxa de operações, influenciam diretamente os resultados: configurações que favorecem alta
cardinalidade de séries beneficiam arquiteturas orientadas a colunas, enquanto configurações de
baixa cardinalidade com lotes grandes podem favorecer bancos relacionais. Por fim, ainda que
todos os bancos de dados sejam executados sequencialmente no mesmo hardware, eliminando
viés de infraestrutura, execuções consecutivas de longa duração expõem o host ao risco de
variação térmica entre escalas; este risco foi mitigado pela adoção de ciclo de vida sequencial
de contêineres, detalhado na Seção~\ref{sec:infrastructure}.
...
A escolha de cada nível reflete cenários de implantação IoT reconhecíveis. A escala pequena
(5 clientes, 10 dispositivos, 10 sensores, 1.000 laços) representa ambientes com poucos
sensores e baixa frequência de amostragem, comuns em projetos piloto ou instalações
residenciais, e serve principalmente para verificar a correção funcional e medir a sobrecarga
base de cada banco de dados. A escala média (5 clientes, 10 dispositivos, 10 sensores,
5.000 laços) corresponde a instalações industriais de porte moderado, onde o volume acumulado
começa a pressionar os mecanismos de compressão e os tamanhos de buffer internos. A escala
grande (10 clientes, 50 dispositivos, 20 sensores, 5.000 laços) simula ambientes de alta
densidade, como cidades inteligentes ou plantas industriais de grande porte, onde o volume de
dados é o principal gargalo e diferenças arquiteturais entre os bancos de dados se tornam
mais pronunciadas.
