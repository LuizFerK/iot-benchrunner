...
dados. A avaliação cobre vazão de escrita, latência por operação (i.e., média e percentil 99) e consumo
de recursos (i.e., CPU e memória) para cada combinação de banco de dados, tipo de carga de trabalho e
escala. O objetivo é caracterizar quais compromissos arquiteturais favorecem quais perfis de
aplicação.

Um objetivo central de projeto, além da medição de desempenho, é a reprodutibilidade.
...
suficientes para replicação independente~\citep{pereira2023mulletbench}. Os demais omitiram as configurações completamente ou as descreveram com poucos detalhes para serem seguidas mecanicamente.
Este trabalho trata a reprodutibilidade como prioridade central: cada componente necessário para replicar os resultados tem sua versão fixada e está publicamente disponível junto com este artigo.

\subsection{Seleção dos Bancos de Dados}

Os três bancos de dados foram selecionados para representar abordagens arquiteturais distintas para o armazenamento de séries temporais, sendo cada um deles relevante na literatura revisada.

\subsubsection{InfluxDB 2.7}

O InfluxDB serve como uma base comum na pesquisa comparativa de BDSTs, aparecendo em quatro dos cinco estudos de desempenho revisados~\citep{naqvi2017influxdb,
shah2022performance,lima2024avaliando,wang2023iotdb}. Ele usa um motor de armazenamento TSM
(Time-Structured Merge Tree) que organiza dados em blocos comprimidos ordenados por tempo,
com escritas absorvidas em cache em memória e compactadas periodicamente em arquivos imutáveis
em disco. A ingestão ocorre via API HTTP com protocolo de linha; consultas analíticas usam a
linguagem de expressão Flux. O InfluxDB mantém um Índice de Séries Temporais (TSI) para
localização de séries; a memória em processo é dominada pelo cache TSM, pelo TSI e pela
sobrecarga do runtime Go. A presença frequente do InfluxDB em benchmarks anteriores o torna um
ponto de referência útil: qualquer novo estudo que o omita perde comparabilidade com a
literatura existente.

\subsubsection{Apache IoTDB 1.3.0}

O Apache IoTDB~\citep{wang2023iotdb} emergiu como líder de desempenho nos dois
...
Desenvolvido na Universidade Tsinghua especificamente para IoT industrial, usa o TsFile, um
formato de arquivo colunar personalizado que armazena dados em blocos ordenados com estatísticas
de mínimo/máximo por bloco. As escritas são absorvidas em buffer no heap gerenciado pela JVM e
descarregadas assincronamente para segmentos TsFile, em uma estratégia similar a uma
Log-Structured Merge Tree (LSM)~\citep{oneil1996lsm}. A ingestão nativa usa um protocolo
binário Thrift RPC (Session API), submetendo tablets colunares tipados diretamente no buffer
de escrita. As estatísticas por bloco permitem a eliminação antecipada de blocos irrelevantes
em consultas de filtro de valor, vantagem que cresce com o volume de dados. Sua rápida ascensão
em benchmarks recentes, combinada com essa abordagem arquitetural distinta, tornou-o adequado
para este estudo.

\subsubsection{TimescaleDB}

O TimescaleDB estende o PostgreSQL~15 com capacidades de séries temporais: particionamento
automático de hipertabela por intervalo de tempo, a função de agregação \texttt{time\_bucket()}
para subamostragem e compressão opcional por bloco. Ele mantém o motor relacional completo com durabilidade baseada em WAL, controle de
concorrência MVCC, índices B-tree e compatibilidade completa com SQL. Os índices BRIN
(Block Range INdex) fornecem eliminação baseada em intervalos para predicados de valor. Para organizações com infraestrutura PostgreSQL existente, ou para aplicações
...
O PostgreSQL, sem a extensão TimescaleDB, foi explicitamente excluído. O TimescaleDB
tende a superar o PostgreSQL neste domínio; incluir ambos duplicaria os testes.
...
A Tabela~\ref{tab:db_comparison} sumariza as principais características dos três bancos
selecionados.

\begin{table}[ht]
  \centering
  \caption{Comparação entre os bancos de dados de séries temporais avaliados.}
  \label{tab:db_comparison}
  \begin{tabular}{lllll}
    \hline
    \textbf{Banco de Dados} & \textbf{Arquitetura} & \textbf{Armazenamento} & \textbf{Protocolo} & \textbf{Consistência} \\
    \hline
    IoTDB      & TSDB nativa orientada a colunas                    & TsFile (formato proprietário)          & IoTDB-SQL, MQTT, REST         & Eventual      \\
    InfluxDB   & TSDB nativa com mecanismo TSM                      & TSM (Time-Structured Merge Tree)       & Line Protocol, HTTP/Flux      & Eventual      \\
    TimescaleDB & Extensão do PostgreSQL com hipertabelas            & Herda estruturas do PostgreSQL         & Protocolo PostgreSQL, HTTP    & ACID completa \\
    \hline
  \end{tabular}
\end{table}
...
O gerador de carga de trabalho é o \textit{iot-benchmark}~\citep{iotbenchmark}, que vem com clientes específicos para os três bancos de dados em
avaliação. Esse suporte nativo a múltiplos bancos de dados significa que cada carga de trabalho
é expressa no protocolo correto para cada alvo: o modo SESSION\_BY\_TABLET do iot-benchmark, que usa a Session API Thrift RPC do IoTDB,
o protocolo de linha HTTP para InfluxDB e uma conexão JDBC para TimescaleDB. Uma
interface consistente e parametrizada garante que as diferenças de desempenho medidas reflitam
os próprios bancos de dados, e não a camada de benchmarking. O iot-benchmark é fundamentado no
YCSB (\textit{Yahoo! Cloud Serving Benchmark}), adaptando sua filosofia de cargas de trabalho
parametrizadas ao contexto de séries temporais IoT~\citep{iotbenchmarkdoc}.
...
momento desta escrita. O Apache JMeter lida bem com sistemas baseados em HTTP, mas requer scripts personalizados para interfaces
Thrift e JDBC, introduzindo potencial sobrecarga específica da ferramenta. A interface uniforme
do iot-benchmark e a semântica de carga de trabalho específica para IoT (i.e., clientes
...
escolha mais adequada para este estudo. Cabe registrar, no entanto, que o iot-benchmark é
desenvolvido pelo mesmo grupo de pesquisa responsável pelo Apache IoTDB~\citep{iotbenchmark},
o que representa um potencial viés em favor desse banco de dados; os resultados relativos ao
IoTDB devem ser interpretados com essa ressalva.
...
\begin{description}
  \item[\texttt{write}] Ingestão sequencial de linha de base com registros ordenados cronologicamente e tamanho de lote padrão de 100 linhas por requisição. Mede a vazão bruta de escrita em condições ideais e ordenadas.

  \item[\texttt{out-of-order}] Vinte por cento dos pontos de dados chegam indexados com tempo anterior à fronteira de ingestão atual, modelado como um atraso distribuído por Poisson. Simula sensores IoT que perdem conectividade e depois reproduzem leituras armazenadas em buffer, um cenário típico em implantações industriais e de campo onde os sensores operam sobre redes celulares ou LPWAN não confiáveis.
...
Cada teste de escrita limpa e repopula o banco de dados antes de ser executado, garantindo
...
A escala grande contém aproximadamente 20 vezes mais dados que a média e cem vezes mais
...
5~$\times$~10~$\times$~10). A medição em múltiplas escalas é essencial porque vários fenômenos -- como a saturação de
compilação JIT da JVM, o esgotamento de buffer, as paralisações de checkpoint e o
escalonamento de varredura de índice -- só se manifestam ou se tornam dominantes em volumes
de dados mais elevados, como mostrado nos resultados da Seção~\ref{sec:scale-effects}. Um
benchmark de escala única arrisca tirar conclusões que se aplicam apenas a um intervalo
estreito de dados.

A escolha de cada nível reflete cenários de implantação IoT. A escala pequena representa ambientes com poucos
sensores e baixa frequência de amostragem, comuns em provas de conceito ou instalações
residenciais e serve principalmente para verificar a correção funcional e medir a sobrecarga
base de cada banco de dados. A escala média corresponde a instalações industriais de porte moderado, em que o volume acumulado
começa a pressionar os mecanismos de compressão e os tamanhos de buffer internos. A escala
grande simula ambientes de alta densidade, como cidades inteligentes ou plantas industriais, onde o volume de
dados é um gargalo e diferenças arquiteturais entre os bancos de dados se tornam
mais pronunciadas.
...
Os experimentos foram conduzidos de forma sequencial, com apenas um banco de dados ativo
por vez, garantindo isolamento completo entre as medições e eliminando interferência de
cargas concorrentes. O Docker Compose~\citep{docker} gerencia os contêineres de banco de
dados com versões de imagem fixadas: \texttt{apache/iotdb:1.3.0-standalone},
\texttt{influxdb:2.7-alpine} e \texttt{timescale/timescaledb:latest-pg15}. Os contêineres
são iniciados e parados sequencialmente: apenas um banco de dados está ativo a qualquer
momento. Esse design, introduzido após as execuções iniciais revelarem artefatos de
contenção de recursos entre bancos de dados, dá a cada banco de dados acesso exclusivo à
CPU, largura de banda de memória e entrada/saída do host.

\subsubsection{Nix Flake}

O Nix flake~\citep{nixos} é uma ferramenta opcional que fornece um ambiente de
...
\subsubsection{CLI Python}

A CLI Python (\texttt{benchmark.py}) orquestra o fluxo de trabalho completo do benchmark
...
O ambiente completo de benchmark está publicamente disponível em
\url{https://github.com/LuizFerK/iot-benchrunner}. Instruções detalhadas
de configuração e execução encontram-se no repositório.
...
\begin{enumerate}
  \item \textbf{Ciclo de vida sequencial de contêineres.} A configuração original iniciava todos
    ...
    largura de banda de entrada/saída~\citep{shapira2016pitfalls}. O TimescaleDB acumulou vários gigabytes de cache de páginas durante
    as sessões de escrita do IoTDB, produzindo medições de memória enganosas. A JVM
  ...
  \item \textbf{Ajuste no nível do banco de dados.} O TimescaleDB recebeu uma configuração
    pgtune~\citep{pgtune} para PostgreSQL~15 direcionada ao hardware desta máquina:
