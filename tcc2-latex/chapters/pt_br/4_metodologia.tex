...
O Apache IoTDB~\cite{wang2023iotdb} emergiu como líder de desempenho nos dois
...
O Docker Compose~\cite{docker} gerencia os contêineres de banco de dados com versões
...
sequencialmente: apenas um banco de dados está ativo a qualquer momento. Esse design,
introduzido após as execuções iniciais revelarem artefatos de contenção de recursos entre bancos
de dados, dá a cada banco de dados acesso exclusivo à CPU, largura de banda de memória e entrada/saída
do host.

\subsubsection{Nix Flake}

O Nix flake~\cite{nixos} é uma ferramenta opcional que fornece um ambiente de
...
retorna exatamente um. A latência é portanto usada como a métrica primária de comparação de
