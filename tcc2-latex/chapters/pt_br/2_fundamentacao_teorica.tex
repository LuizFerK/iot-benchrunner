...
alternativa de armazenar dados em arquivos de dados simples e escrever código de gerenciamento específico
...
Os bancos de dados relacionais organizam os dados em tabelas compostas por linhas e colunas,
nas quais cada tabela representa uma entidade e cada linha corresponde a um registro dessa
entidade. A interação com esses sistemas é realizada por meio da linguagem SQL, que permite
definir, manipular e consultar os dados de forma padronizada. Uma propriedade central dos
bancos de dados relacionais são as garantias ACID (Atomicidade, Consistência, Isolamento e
Durabilidade), que asseguram a integridade das transações mesmo em cenários de falha ou acesso
concorrente, tornando-os amplamente adotados em aplicações corporativas que exigem consistência
estrita dos dados. No contexto deste trabalho, o TimescaleDB, um dos bancos de dados avaliados,
é implementado como uma extensão do PostgreSQL, um dos SGBDs relacionais de código aberto mais
consolidados, herdando toda a sua pilha de armazenamento e garantias de consistência.
...
reduz a contenção de CPU e entrada/saída, enquanto o paralelismo pode ser explorado tanto entre consultas
...
A Figura~\ref{fig:temporal_data} ilustra uma série temporal de medições meteorológicas. Cada ponto no gráfico corresponde a uma leitura realizada em um instante específico: o eixo horizontal representa a progressão do tempo, enquanto o eixo vertical representa o valor medido. Essa estrutura evidencia a natureza append-only dos BDSTs, nos quais os dados não são alterados retrospectivamente, mas acumulados continuamente ao longo do tempo.
...
leituras em um momento específico. Observa-se que o campo de tempo funciona como chave primária natural, permitindo que consultas por intervalo temporal sejam atendidas com varreduras contíguas em disco: é exatamente essa propriedade estrutural que confere aos BDSTs desempenho superior frente aos bancos relacionais de propósito geral para esse padrão de acesso.
...
conjuntos de dados de séries temporais são tipicamente utilizados em situações em que as medições,
...
abordagens principais para o armazenamento de séries temporais: (1) tabelas com muitas colunas que
armazenam dados ponto a ponto; (2) um design híbrido misturando tabelas com muitas colunas com armazenamento
em bloco binário de dados (BLOB, do inglês \textit{Binary Large Object}); e (3) inserção direta de BLOB a partir de um cache em memória, que pode atingir taxas
de ingestão até 1.000 vezes mais rápidas que as abordagens ponto a ponto. Os BDSTs armazenam
...
\textbf{Apache IoTDB}~\citep{wang2023iotdb} é um banco de dados de séries temporais de código
aberto, desenvolvido pela Universidade Tsinghua e mantido pela Apache Software Foundation,
concebido especificamente para cargas de trabalho IoT industriais. Ele adota um formato de
armazenamento colunar proprietário (TsFile) e oferece alta vazão de ingestão por meio de uma
interface Thrift RPC nativa. Os detalhes arquiteturais são apresentados na
Seção~\ref{sec:methodology}.

\textbf{InfluxDB}~\citep{naqvi2017influxdb} usa um motor de armazenamento Time-Structured Merge
...
automaticamente particionada em blocos físicos por intervalo de tempo. Consultas que
...
A Tabela~\ref{tab:db_comparison} sumariza as principais características dos três bancos de dados.
...
três qualidades essenciais. Primeiro, portabilidade: a capacidade de executar o
benchmark em diferentes plataformas e sistemas. Segundo, clareza: garantir fácil
compreensão do que está sendo realmente medido. Terceiro, escalabilidade: a adaptação
...
Os benchmarks de Processamento de Transações Online (TPC-A, TPC-B, TPC-C) definem
...
gama muito mais ampla de capacidades do sistema do que seus predecessores. O TPC-C foi concebido
para cargas OLTP (Online Transaction Processing), enquanto o TPC-H foca em cargas OLAP (Online
Analytical Processing), evidenciando que benchmarks distintos são necessários para avaliar
diferentes perfis de acesso a dados. Os benchmarks de
desempenho de consultas (Wisconsin, AS3AP, TPC-D) avaliam o desempenho de consultas
relacionais, de simples a complexas para suporte a decisões. Os benchmarks de banco de
...
\cite{shapira2016pitfalls} catalogam cinco armadilhas recorrentes em benchmarks de sistemas de
big data: comparar configurações que diferem em múltiplos parâmetros simultaneamente, testar
em escala insuficiente e extrapolar os resultados, aceitar resultados suspeitos sem questionar
sua origem, usar cargas de trabalho artificiais desconectadas do uso real, e comunicar
resultados de forma incompleta ou enganosa. Essas armadilhas são particularmente relevantes
para benchmarks de bancos de dados de séries temporais, onde diferenças de configuração,
isolamento de recursos e escolha de carga de trabalho podem inverter conclusões.
...
uma lacuna que este estudo aborda diretamente por meio de seu ambiente de benchmark
totalmente automatizado e publicamente disponível.
...
hardware introduz artefatos de contenção de cache de páginas e entrada/saída que podem inverter
