...
tradicionais: independência de dados (i.e., os programas de aplicação são isolados dos detalhes de
representação e armazenamento de dados); acesso eficiente aos dados (i.e., por meio de técnicas
sofisticadas de indexação e otimização de consultas); integridade e segurança dos dados (i.e., o SGBD
...
como uma coleção de relações (i.e., tabelas com linhas e colunas), uma representação tão intuitiva
que mesmo usuários não especialistas conseguem entender o conteúdo armazenado~\citep{Ramakrishnan2003}.
A estrutura de cada relação consiste em um esquema (i.e., nomes dos campos e seus domínios) e uma
instância (i.e., o conjunto de tuplas, ou registros). Uma propriedade essencial é que cada relação é
...
são divididas entre clientes (i.e., os responsáveis pela interface do usuário) e servidores (i.e., os responsáveis
...
(i.e., interqueries) quanto dentro de uma única consulta (i.e., intraquery). No entanto, isso traz desafios:
...
A Figura~\ref{fig:temporal_data} ilustra uma série temporal de medições meteorológicas. Cada ponto no gráfico corresponde a uma leitura realizada em um instante específico. O eixo horizontal representa a progressão do tempo, enquanto o eixo vertical representa o valor medido.
...
\textbf{Apache IoTDB}~\citep{wang2023iotdb} foi concebido especificamente para o domínio IoT.
...
de escrita. A interface de ingestão nativa é um protocolo binário Thrift RPC (Session API), que submete
tablets colunares tipados diretamente no buffer de escrita com sobrecarga mínima. As estatísticas de mínimo/máximo armazenadas por bloco permitem a eliminação antecipada de blocos incompatíveis durante consultas de filtro de valor, uma propriedade que se torna cada vez mais vantajosa à medida que o conjunto de
