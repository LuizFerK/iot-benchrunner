...
Da leitura inicial, sete trabalhos foram selecionados para estudo detalhado. Com base nestes trabalhos, foram identificadas duas linhas de análise: (i) Desempenho e (ii) Comparativa. Desta forma, este capítulo está organizado em duas seções. A seção de Análise de Desempenho cobre estudos comparativos de benchmark. A seção de Análise Comparativa sintetiza e critica todos os trabalhos selecionados por meio de uma tabela comparativa, destacando padrões, lacunas e tendências identificadas na literatura.

\subsection{Análise de Desempenho}

Seis trabalhos no formato de benchmarks controlados ou comparações em cenários reais foram
...
\cite{grzesik2020comparative} avaliaram cinco bancos de dados de séries temporais (TimescaleDB,
InfluxDB, Riak~TS, PostgreSQL e SQLite) em um Raspberry~Pi~4 (ARM Cortex-A72, 4\,GB de RAM),
escolhido como dispositivo de borda de baixo consumo representativo, sendo o único estudo
na literatura revisada direcionado a implantações com recursos restritos. Os testes cobriram
inserções pontuais e em lote (grupos de~10) e três cenários de consulta de agregação executados
5.000 vezes cada, utilizando 28.800 leituras sintéticas de sensores (temperatura, pressão,
umidade e métricas de qualidade do ar de 10 sensores simulados em uma janela de 24 horas).
O PostgreSQL foi o sistema mais rápido tanto para inserção quanto para consulta nessas restrições
de hardware; o InfluxDB apresentou o maior ganho relativo com o uso de lotes (melhoria de
8,65 vezes na vazão). TimescaleDB e InfluxDB foram destacados como escolhas mais adequadas
quando recursos nativos de séries temporais, como políticas de retenção e agregados contínuos,
são necessários em implantações de borda em produção. A reprodutibilidade foi classificada
como inexistente, sem scripts ou configurações publicados.
...
A Tabela~\ref{tab:related} agrega os principais atributos dos seis estudos de desempenho,
...
\begin{table*}[t]
  ...
  \begin{tabular}{lllllll}
    ...
    \cite{grzesik2020comparative}  & 2020 & SQL + NoSQL & Nenhuma & Sintético          & Manual                        & PostgreSQL \\
...
\subsubsection{Cobertura de Bancos de Dados}

O InfluxDB está presente em todos os estudos revisados, servindo como o ponto de referência
de facto para comparações de BDSTs, mesmo em trabalhos onde outro sistema é o assunto
principal, como em~\citep{wang2023iotdb}. A presença do TimescaleDB em quatro dos seis estudos
reflete sua crescente relevância como alternativa compatível com SQL. A tendência mais notável
é a emergência do Apache IoTDB: aparecendo apenas nos dois estudos mais recentes (2023), ele
lidera em ambos~\citep{pereira2023mulletbench,wang2023iotdb}, sugerindo melhorias de desempenho
nas versões recentes, viés de seleção dos benchmarks mais recentes para cargas de trabalho IoT,
ou um crescente reconhecimento de seus pontos fortes para esse domínio; os dados disponíveis
não permitem distinguir entre essas explicações.

\subsubsection{Reprodutibilidade}

Esta é a lacuna metodológica mais significativa na literatura revisada. Dos seis estudos de
desempenho, apenas um (\citep{pereira2023mulletbench}) é totalmente reprodutível, com todo o
código e configurações publicamente disponíveis no GitHub. Dois estudos
(\citep{lima2024avaliando} e \citep{wang2023iotdb}) oferecem reprodutibilidade parcial por meio
de contêineres Docker ou configurações descritas, enquanto os dois restantes
(\citep{shah2022performance} e \citep{naqvi2017influxdb}) não fornecem nenhum meio de replicação.
...
Em contraste, o presente trabalho foi desenvolvido com reprodutibilidade como requisito central
do projeto. O ambiente experimental completo é definido por meio de um Nix flake, que garante
dependências determinísticas e elimina variações causadas por diferenças de sistema operacional
ou versões de bibliotecas. Cada banco de dados executa em contêiner Docker isolado com imagem
de versão fixada, impedindo atualizações silenciosas que alterem o comportamento observado. A
ferramenta de automação dos testes, o iot-benchrunner, bem como todos os arquivos de
configuração utilizados, estão publicamente disponíveis em
\url{https://github.com/LuizFerK/iot-benchrunner}. Os testes são executados de forma
sequencial, e não concorrente, eliminando interferência entre instâncias de banco de dados
durante a coleta de métricas. Os trabalhos relacionados, por sua vez, descrevem o ambiente
experimental apenas em prosa, sem fornecer a configuração efetiva utilizada, além de dependerem
de hardware proprietário ou de versões de ferramentas que podem não estar mais disponíveis,
o que inviabiliza a replicação independente dos experimentos.

\subsubsection{Conjuntos de Dados}

Todos os seis estudos utilizam conjuntos de dados sintéticos, refletindo a necessidade de
dados controlados e escaláveis em volume. Três estudos incluem adicionalmente conjuntos de
dados reais, como dados de táxi em NYC, leituras de sensores IoT e dados industriais de
veículos~\citep{shah2022performance,naqvi2017influxdb,wang2023iotdb}, proporcionando relevância
prática ao lado do controle experimental. O TSBS (Time Series Benchmark Suite) aparece nos
estudos mais recentes, sugerindo uma tendência potencial de padronização na comunidade.

\subsubsection{Configurações de Hardware}

Os ambientes testados variam consideravelmente: os processadores variam de Intel~i3 a
Intel~i7, a RAM de 8\,GB a 32\,GB (mais nuvem com 256\,GB em um caso~\citep{wang2023iotdb}),
e o armazenamento de SATA SSD a NVMe e HDD. A maioria dos testes utilizou máquinas de médio
porte, consistente com foco em cenários de implantação realistas. A inclusão de testes em nuvem
em estudos mais recentes~\citep{pereira2023mulletbench,wang2023iotdb} reflete o crescente
interesse em arquiteturas borda-nuvem.

\subsubsection{Tendências de Resultados}

O InfluxDB lidera em três dos seis estudos (2017, 2022 e 2024). O Apache IoTDB lidera em
ambos os estudos de 2023. O PostgreSQL lidera no estudo de computação de
borda~\citep{grzesik2020comparative}, resultado atribuível ao hardware restrito do
Raspberry~Pi em vez de características gerais de desempenho de BDSTs. O padrão temporal entre
os benchmarks de propósito geral permanece evidente: pode indicar melhorias de desempenho nas
versões recentes do IoTDB, ou pode refletir o fato de que benchmarks mais recentes incluem
cargas de trabalho específicas de IoT nas quais o modelo de dados nativo do IoTDB oferece uma
vantagem estrutural. A mudança não invalida necessariamente o InfluxDB para outras cargas de
trabalho; no entanto, motiva a inclusão de ambos os bancos de dados em qualquer avaliação
comparativa contemporânea.

A literatura revisada revela três lacunas principais que o presente trabalho aborda:
(1) reprodutibilidade insuficiente: apenas um dos seis estudos é totalmente replicável;
