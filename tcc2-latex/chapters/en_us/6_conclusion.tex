...
and publicly reproducible benchmark infrastructure built on Docker Compose and
...
TimescaleDB's \texttt{time\_bucket()} downsampling query stays below 1\,ms across a hundredfold
increase in data volume, making it the strongest choice for dashboard analytics, while its WAL-based
...
a hundredfold increase in data) because its chunk min/max statistics provide better block-level
...
queries scan a fixed time window, not the full dataset. These behaviors derive from each system's core architectural choices and are therefore
more stable than gains obtained through configuration tuning. Version changes that alter
those core choices, such as the columnar engine introduced in InfluxDB~3.0, may shift
the trade-offs observed here.
...
they can be presented as architectural properties~\citep{shapira2016pitfalls}.
...
\begin{table*}[t]
  ...
  \label{tab:final}
  \begin{tabular}{l c c c p{0.3\linewidth}}
    \hline
    ...
    Out-of-order throughput (abs.) & \textbf{H} & L & L & 1.87\,M / 255\,K / 160\,K pts/s; IoTDB leads at every scale \\
    ...
    Memory footprint (write)      & L & \textbf{H} & H & $\sim$1.7\,GB / $\sim$190\,MB / $\sim$239\,MB start \\
...
\subsubsection{Single-Node Deployment}

All databases ran on one host, eliminating real network
...
\subsubsection{Sequential Read Tests}

Read workloads run sequentially on the same dataset within
...
\subsubsection{IoTDB Deferred Durability}

IoTDB's write throughput reflects in-memory
...
\subsubsection{JVM Warm-Up}

IoTDB's read and single-point write performance improves significantly
over the duration of a session as JIT compilation optimises hot paths~\citep{barrett2017warmup}. Short benchmarks
