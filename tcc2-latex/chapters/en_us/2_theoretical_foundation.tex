...
note, storing data near its points of use reduces CPU and input/output contention, while parallelism
...
a memory cache, which can achieve ingestion rates up to 1,000 times faster than
...
write throughput. The native ingestion interface is a binary Thrift RPC protocol (Session API), which submits
typed columnar tablets directly into the write buffer with minimal overhead. The min/max statistics stored per chunk enable early pruning of non-matching blocks during value-predicate queries, a property that becomes increasingly
...
partitioned into physical chunks by time interval. Queries that specify a time range
...
three essential qualities. First, portability: the ability to run the benchmark on
different platforms and systems. Second, clarity: ensuring easy understanding of what
is actually being measured. Third, scalability: the natural adaptation to larger
...
Online Transaction Processing benchmarks (TPC-A, TPC-B, TPC-C) define standard
...
range of system capabilities than its predecessors. Query performance benchmarks
(Wisconsin, AS3AP, TPC-D) evaluate relational query performance from simple to complex
decision-support workloads. Object database benchmarks (001, 007, Bucky) measure
...
\cite{shapira2016pitfalls} catalog five recurring pitfalls in big data system benchmarks:
comparing configurations that differ in multiple parameters simultaneously, testing at
insufficient scale and extrapolating results, accepting suspicious results without questioning
their origin, using artificial workloads disconnected from real usage, and reporting results
incompletely or misleadingly. These pitfalls are particularly relevant for time series database
benchmarks, where configuration differences, resource isolation, and workload choice can
reverse conclusions.
...
addresses through its fully scripted and publicly available benchmark environment.
Beyond code sharing, resource isolation between benchmark systems is equally critical:
running multiple database engines on shared hardware introduces page-cache and input/output
