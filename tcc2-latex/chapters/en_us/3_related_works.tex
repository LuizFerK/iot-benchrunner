...
From the initial reading, seven works were selected for detailed study. Based on these works, a characterisation of three lines of work was made: (i) Functionality, (ii) Performance, and (iii) Comparative. This chapter is organized
in three sections. The Functionality Analysis section covers works that evaluate
technical and operational characteristics of TSDB systems. The Performance Analysis
section covers comparative benchmark studies. The Comparative Analysis
section synthesizes and critiques all selected works through a comparative table, highlighting patterns,
...
Six works in the format of controlled benchmarks or real-scenario comparisons were selected
...
\cite{grzesik2020comparative} evaluated five time series databases (TimescaleDB, InfluxDB,
Riak~TS, PostgreSQL, and SQLite) on a Raspberry~Pi~4 (ARM Cortex-A72, 4\,GB RAM), chosen
as a representative low-power edge device, making this the only study in the reviewed
literature targeting constrained-resource deployments. Tests covered single-point and batch
inserts (groups of~10) and three aggregation query scenarios executed 5{,}000 times each,
using 28{,}800 synthetic sensor readings (temperature, pressure, humidity, and air-quality
metrics from 10 simulated sensors over a 24-hour window). PostgreSQL was the fastest system
for both insert and query workloads under these hardware constraints; InfluxDB showed the
largest relative gain from batching (8.65 times throughput improvement). TimescaleDB and
InfluxDB were highlighted as more practical choices when native time-series features such as
retention policies and continuous aggregates are required in production edge deployments.
Reproducibility was classified as non-existent, with no scripts or configurations published.
...
Table~\ref{tab:related} aggregates the key attributes of the six performance studies, enabling
...
\begin{table*}[t]
  ...
  \begin{tabular}{lllllll}
    ...
    \cite{grzesik2020comparative}  & 2020 & SQL + NoSQL & None    & Synthetic          & Manual                        & PostgreSQL \\
...
\subsubsection{Database Coverage}

InfluxDB is present in every reviewed study, serving as the de
facto reference point for TSDB comparisons, even in works where another system is the
primary subject, as in~\cite{wang2023iotdb}. The presence of TimescaleDB in four of six
...
This is the most significant methodological gap in the reviewed
literature. Of six performance studies, only one, \cite{pereira2023mulletbench},
...
All six studies use synthetic datasets, reflecting the need for
...
Tested environments vary considerably: processors range
...
\subsubsection{Outcome Trends}

InfluxDB leads in three of six studies (2017, 2022, and 2024).
Apache IoTDB leads in both 2023 studies. PostgreSQL leads in the edge-computing
study~\citep{grzesik2020comparative}, a result attributable to the constrained Raspberry~Pi
hardware rather than general TSDB performance characteristics. The temporal pattern among
general-purpose benchmarks remains notable: it may indicate performance improvements in
IoTDB's recent versions, or it may reflect the fact that more recent benchmarks include
IoT-specific workloads where IoTDB's native data model provides a structural advantage.
The shift does not necessarily invalidate InfluxDB for other workloads; it does, however,
motivate the inclusion of both databases in any contemporary comparative evaluation.

The reviewed literature reveals three key gaps that the present work addresses:
(1) insufficient reproducibility: only one of six studies is fully replicable; (2) limited
