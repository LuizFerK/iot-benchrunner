...
characterize which architectural trade-offs favour which application profiles.

A central design objective beyond performance measurement is reproducibility.
...
configurations entirely or described their setups in too little detail to follow mechanically.
This work treats reproducibility as a central priority: every component needed to replicate the results is version-pinned and publicly available
alongside this paper.

\subsection{Database Selection}

The three databases were selected to represent distinct architectural approaches to time-series storage, each being relevant in the reviewed literature.

\subsubsection{InfluxDB 2.7}

InfluxDB serves as a common baseline in TSDB comparative research, appearing in four of the five performance studies reviewed~\citep{naqvi2017influxdb,
...
InfluxDB's frequent presence in prior benchmarks makes it a useful reference point: any new
study that omits it loses comparability with the existing literature.

\subsubsection{Apache IoTDB 1.3.0}

Apache IoTDB~\cite{wang2023iotdb} emerged as the performance leader in
...
file format (TsFile) and a binary Thrift RPC protocol (Session API). Write ingestion
...
recent benchmarks, combined with its distinct architectural approach, made it suitable for this study.

\subsubsection{TimescaleDB}

TimescaleDB extends PostgreSQL~15 with time-series capabilities: automatic
...
The workload generator is \textit{iot-benchmark}~\citep{iotbenchmark}, which ships purpose-built benchmark clients for all three databases under evaluation.
This native multi-database support means each workload is expressed in the correct protocol
for each target: iot-benchmark's SESSION\_BY\_TABLET mode, which uses IoTDB's Thrift RPC Session API,
the HTTP line-protocol for InfluxDB, and a JDBC connection for TimescaleDB. A consistent, parameterized interface
ensures that differences in measured performance reflect the databases themselves rather
than the benchmarking layer. iot-benchmark is grounded in
YCSB (\textit{Yahoo! Cloud Serving Benchmark}), adapting its philosophy of parameterized workloads
to the IoT time series context~\citep{iotbenchmarkdoc}.
...
\begin{description}
  \item[\texttt{write}] Baseline sequential ingestion with chronologically ordered records and the
    default batch size of 100 rows per request. This measures raw sustained write throughput
    under ideal, ordered conditions.

  \item[\texttt{out-of-order}] Twenty percent of data points arrive indexed with a timestamp
    earlier than the current ingestion frontier, modeled as a Poisson-distributed delay. This simulates
    IoT sensors that lose connectivity and later replay buffered readings, a typical scenario
    in industrial and field deployments where sensors operate over unreliable cellular
    or LPWAN networks.
...
Large scale contains approximately 20 times more data than medium and one hundred times more
...
measurement is essential because several phenomena -- such as JVM JIT compilation
saturation, buffer exhaustion, checkpoint stalls, and index scan scaling -- only
manifest or become dominant at higher data volumes, as shown in the results of Section~\ref{sec:scale-effects}. A single-scale benchmark risks drawing
...
Docker Compose~\cite{docker} manages the database containers with pinned image
...
sequentially: only one database is active at any given time. This design, introduced
after the initial runs revealed cross-database resource contention artefacts, gives each
database exclusive access to the host's CPU, memory bandwidth, and input/output.

\subsubsection{Nix Flake}

The Nix flake~\cite{nixos} is an optional tool that provides a declarative,
...
\subsubsection{Python CLI}

The Python CLI (\texttt{benchmark.py}) orchestrates the complete benchmark workflow
...
The complete benchmark environment is publicly available at
\url{https://github.com/LuizFerK/iot-benchrunner}. Detailed setup and
execution instructions can be found in the repository.
...
hundreds of points, a latest-point query returns exactly one. Latency is therefore
...
\begin{enumerate}
  \item \textbf{Sequential container lifecycle.} The original setup started all three
    ...
    containers competed for OS page cache and input/output bandwidth~\citep{shapira2016pitfalls}. TimescaleDB accumulated several
    gigabytes of page cache during IoTDB's write sessions, producing misleading memory
  ...
  \item \textbf{Database-level tuning.} TimescaleDB received a pgtune~\citep{pgtune}
