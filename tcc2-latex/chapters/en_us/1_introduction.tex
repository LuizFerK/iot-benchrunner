...
Beyond throughput, TSDBs offer specialized compression mechanisms, time-interval data
co-location, and automatic retention policies that reduce operational costs, features that
are important for IoT applications where continuous sensor measurements demand scalability
...
architectural approaches to the same problem. \textit{Apache IoTDB}~\citep{wang2023iotdb},
...
extends PostgreSQL with time-series capabilities, offering SQL compatibility
...
reproducibility. A survey of recent comparative studies reveals that a large portion of published
...
environment (all orchestration scripts, container definitions, build environment, and
...
\begin{itemize}
  ...
  \item Provide a reproducible benchmark environment that enables future researchers to
